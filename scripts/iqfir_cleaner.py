...
def modelsim_to_csv(input_file, output_file):
    ...
    with open(input_file, 'r') as f:
        # On saute les premières lignes d'en-tête
        lines = f.readlines()
        
        for line in lines:
            parts = line.split()
            # On vérifie 4 colonnes (Temps, Delta, I, Q)
            if len(parts) == 4:
                # On tente de convertir I et Q, on ignore si pas binaire
                try:
...
# Utilisation
modelsim_to_csv("scripts/iqfir_data.txt", "scripts/iqfir_data.csv")