...
begin
    ...
    begin
        if rising_edge(clk) then
            ...
            elsif sa_axis_tvalid = '1' and sb_axis_tvalid = '1' and m_axis_tready = '1' then  -- If Output ready and NCO/FIR valid
                icos := sa_axis_cos_tdata * sb_axis_i_tdata;
                qsin := sa_axis_sin_tdata * sb_axis_q_tdata;
                m_axis_tdata <= icos(18 downto 7) + qsin(18 downto 7);  -- Truncature (adapted to signal dynamic)
                m_axis_tvalid <= '1';
            else
                m_axis_tvalid <= '0';
                m_axis_tdata <= (others => '0');
